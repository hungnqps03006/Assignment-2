...
Public Class frmSanPham
    ...
    Private Sub LoadData()
        Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
        conn.Open()
        Dim refesh As SqlDataAdapter = New SqlDataAdapter("select * from SANPHAM", conn)
        db.Clear()
        refesh.Fill(db)
    ...
    Private Sub btnSua_Click(sender As Object, e As EventArgs) Handles btnSua.Click
        ...
        ElseIf btnSua.Text = "Update" Then
            Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
            Dim query As String = "update SANPHAM set TenSP=@TENSP, Soluong=@SOLUONG, maloai=@maloai where MaSP=@MASP"
            Dim save As SqlCommand = New SqlCommand(query, conn)
            conn.Open()
            save.Parameters.AddWithValue("@MASP", txtMaSP.Text)
            save.Parameters.AddWithValue("@TENSP", txtTenSP.Text)
    ...
    Private Sub DataGridView1_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick
        ...
        txtmaloai.Text = DataGridView1.Item(2, click).Value
        txttenloai.Text = DataGridView1.Item(3, click).Value
